...
1. If you don't have a vault deployed for your organization already then reach out and request a demo vault.

<a class="md-button md-button--primary" href="https://anontech.io/contact-us" target="_blank">Request a Vault</a>

2. When you get your vault you will need the web address of your demo vault.
      1. For cloud hosted vaults this takes the form of http://vault.anontech.io/VAULT_IDENTIFIER/ where vault identifier is a hexadecimal code identifying your organizations vault
      2. For locally hosted vaults your administrator can prove you with the root URL to your hosted vizivault instance.

3. Download the example CSV file

## Get your encryption key pair

Take your encryption key and place it into a file. You may wish to secure your private key further in a production environment. Even though we deliver our data securely to your system, you may wish to take precautions to secure these keys in your environment.

## Initialize your connection to the vault

With your decryption key, encryption key, path and application key we can initialize the connection to your vault. Note for the purposes of this quickstart we provide the connection with the encryption and decryoption key. Howver in your application you may only wish to pull down data and decrypt with the decryption key or create new data with the encryption key. Using application keys and access to encyption and decryption keys lets you manage exactly how data is accessed and controlled.
...
3. Replace "https://my.host:8080' with the secure http address and port of your provided vault.
    ...
    #You'll need this later to laod the CSV sample file
    import csv 
...
The first thing we will need to do is establish attributes to store all of the data from our client. Attributes definitions defining the structure and rules data will be governed and stored by. We can tell vizivault if a user can have more than one value of an attribute. If the data should be indexed for cross-user search functionality. We can also associate tags and regualtions directly here establishing how Vizivault will treat the data for retention, storage, and sharing.

We start with creating some very simple attributes with no structure. These can be strings or numeric data. The important thing about these attributes is they are flat and self-contained. In the next example we will see how to handle data where you only may be interested in retrieving part of a related set of data. The hint parameter is a reminder about the data format as a clue to developers about what the4 format of data should look like.
...
* file: For data that takes[index](../index.md) more than 126 characters to represent, such as long text files, or base64-encoded representations of image files.
...
Now that we have data in the system, Let's try to get our data back. Here we grab the data for user 101, but you can pull the data for any user.
