...
|attribute|String|The key of this attribute's [definition](/glossary/attribute-definition)|Required for `"store"` events; optional for other events as it can be derived from the data point id.|
|value|(varies)|The value of this attribute. What type the value is depends on the [schema](/tutorials/attribute-schemas) of the attribute definition. This is used by the [rules engine](/tutorials/rules) and will **not** be stored. |Optional for `"store"` and `"update"` events, ignored for other events|
